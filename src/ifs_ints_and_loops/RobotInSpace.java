...
if (keyPressed==38) {System.out.println(keyPressed);
        rob.setAngle(0);rob.microMove(38);
}
		// 4. If the down arrow is pressed, move the Robot down.
if (keyPressed==40) {
	 rob.setAngle(180);rob.microMove(40);
}
		// 5. If the left arrow is pressed, make the Robot go left.
if (keyPressed==37) {

	rob.setAngle(270);rob.setAngle(270);
}

		// 6. If right is pressed, move the Robot right.
if(keyPressed==39) {
	rob.setAngle(90);rob.setAngle(90);
